...
// === ГЕНЕРАТОР ОТМАЗОК МАЛЫША ===
function getMalyshErrorReply(errText) {
    ...
    if (error.includes('prohibited') || error.includes('safety') || error.includes('blocked') || error.includes('policy')) {
        const phrases = [
            "🤬 Админ опять включил моралиста и зацензурил мой ответ. Сказал, что мы тут слишком токсичные. Сорян.",
            "🔞 Не, ну это бан. Нейронка не хочет это генерить, говорит: Слишком грязно даже для меня.",
            "👮‍♂️ Опа, цензура подъехала. Админ считает, что этот контент оскорбляет чьи-то нежные чувства. Попробуй спросить помягче."
        ];
    ...
    if (error.includes('503') || error.includes('overloaded') || error.includes('unavailable') || error.includes('timeout')) {
        const phrases = [
            "🔥 Там уже сервера плавятся. Подожди минуту, пусть остынут.",
            "🐌 Гугл тупит страшно. Я то запрос кинул, а там тишина. Походу, китайцы опять все видеокарты заняли.",
            "💤 Чёт нейронка устала. Дай перекурить пару секунд."
        ];
        return phrases[Math.floor(Math.random() * phrases.length)];
    }
    ...
    if (error.includes('429') || error.includes('quota') || error.includes('exhausted') || error.includes('лимит')) {
        const phrases = [
            "💸 Всё, ребят, лимиты всё. Мы слишком много болтаем, Гугл перекрыл нам краник. Ждем отката квоты.",
            "🛑 Стопэ. Ошибка 429 — \"Too Many Requests\". Я слишком быстро отвечаю, меня притормозили. Ща отдышусь.",
            "📉 Админ сказал «Хватит болтать. Иди поторгуй». Чисто от меня (шепотом) - попробуй позже)."
        ];
        return phrases[Math.floor(Math.random() * phrases.length)];
    }
    ...
    if (error.includes('400') || error.includes('too large') || error.includes('invalid argument')) {
        const phrases = [
            "🐘 Ты мне библиотеку что ли скинул? Файл слишком жирный, я это не переварю.",
            "📜 Много буков. «Ошибка-ошибка». Сократи басню, братан, в башку не лезет.",
            "💾 Где ты это взял? Давай что-то полегче, да"
        ];
        return phrases[Math.floor(Math.random() * phrases.length)];
    }
    ...
    const phrases = [
        "🛠 Так, у меня шестеренки встали. Какая-то дичь. Админ, просыпайся, мы тут всё сломали!",
        "💥 Я упал. Критическая ошибка! Поднимите кто-нибудь. И позовите админа, я работать не могу.",
        "🚑 Хьюстон, у нас проблемы. Я поймал баг - и не знаю, что с ним делать. Админ, помогай."
    ];
    return phrases[Math.floor(Math.random() * phrases.length)];
}
...
function escapeHtml(text) {
    return text.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
      ...
      async function processBuffer(chatId) {
          ...
          // Фильтруем буфер, чтобы исключить сообщения от бота
          const filteredBuffer = buffer.filter(m => m.userId !== config.botId);
          ...
          const userIds = [...new Set(filteredBuffer.map(m => m.userId))];
          const currentProfiles = storage.getProfilesForUsers(chatId, userIds);
          const updates = await ai.analyzeBatch(filteredBuffer, currentProfiles);
      ...
      async function processChatBuffer(chatId) {
          ...
          // Фильтруем буфер, чтобы исключить сообщения от бота
          const filteredBuffer = buffer.filter(m => !m.name.includes('Малыш'));
          ...
          const updates = await ai.analyzeChatProfile(filteredBuffer, currentProfile);
...
**Почему так?**
Бот работает локально. Поэтому он работает только там, где есть я (в чатах) или в моей личке.`;
   ...
   if (msg.voice || msg.audio) {
    ...
    try {
        ...
        if (transcription && transcription.text) {
            // Проверяем, есть ли обращение "Малыш" в тексте
            const hasTriggerWord = config.triggerRegex.test(transcription.text.toLowerCase());

            if (hasTriggerWord) {
                // Если есть "Малыш" — отвечаем текстом через AI (без показа транскрибации)
                console.log(`[VOICE] Обнаружено обращение "Малыш" в голосовом. Отвечаю текстом.`);

                // Добавляем в историю
                addToHistory(chatId, userName, transcription.text);
                ...
                try {
                    const aiResponse = await ai.getResponse(
                        chatHistory[chatId] || [],
                        { sender: userName, text: transcription.text, replyText: "" },
                        null, // imageBuffer
                        "image/jpeg",
                        "", // instruction
                        userProfile,
                    ...
                    if (aiResponse) {
                        try {
                            await bot.sendMessage(chatId, aiResponse, { reply_to_message_id: msg.message_id, parse_mode: 'Markdown' });
                            console.log(`[VOICE] Ответ отправлен в чат ${chatId}`);
                        } catch(e) {
                            console.error(`[VOICE ERROR] Не смог отправить сообщение: ${e.message}`);
                        }
                    } else {
                        console.error(`[VOICE ERROR] AI вернул пустой ответ`);
                    }
                } catch (e) {
                    stopTyping();
                    console.error(`[VOICE ERROR] Ошибка при получении ответа: ${e.message}`);
                    // Показываем ошибку пользователю
                    try {
                        await bot.sendMessage(chatId, getMalyshErrorReply(e.message), { reply_to_message_id: msg.message_id, parse_mode: 'Markdown' });
                    } catch(sendErr) {}
                }
            } else {
                // Если нет "Малыш" — показываем транскрибацию и суть
                let replyText = "";
...
**💰 Котировки (AllTick WS + REST + Backup):**
• "Малыш цена золота" — GOLD 🥇 *(AllTick REST)*
• "Малыш курс евро" — EURUSD 💶
• "Малыш фунт доллар" — GBPUSD 💷
• "Малыш доллар иена" — USDJPY 💴
• "Малыш австр доллар" — AUDUSD 🇦🇺
• "Малыш цена серебра" — SILVER 🥈
• "Малыш нефть" — WTI 🛢️
• "Малыш биткоин" — BTCUSD ₿ *(CoinGecko)*
• /gold, /eur, /gbp, /jpy, /aud, /silver, /oil, /btc — быстрые команды

**👶 Вижу и Слышу:**
• Кидай голосовое — расшифрую текст и напишу краткую суть.
...
• «Малыш, напомни завтра в 10» — поставлю напоминание. Можно реплаем на сообщение с датой.

**🎲 Развлекуха:**
• "Малыш кинь монетку" — Орёл/Решка.
• "Малыш число 1-100" — Рандомное число в диапазоне.
• "малыш кто из нас [вопрос]" — Выберу случайного из чата.

**🕵️ Досье и Память:**
• "Малыш кто я?" — Моё честное мнение о тебе.
• "Малыш расскажи про @юзера" — Выдам досье на участника.
• "Малыш стата" — Статистика токенов за сутки.
• "Малыш, этот чат про [тема]" — Задать тему чата вручную.
...
_ver: ${config.version}_
  ...
  // === БЫСТРЫЕ КОМАНДЫ КОТИРОВОК ===
  const quickQuoteCommands = {
    '/gold': 'GOLD', '/золото': 'GOLD',
    '/eur': 'EURUSD', '/евро': 'EURUSD',
    '/gbp': 'GBPUSD', '/фунт': 'GBPUSD',
    '/jpy': 'USDJPY', '/иена': 'USDJPY',
    '/aud': 'AUDUSD', '/австр': 'AUDUSD',
    '/silver': 'SILVER', '/серебро': 'SILVER',
    '/oil': 'WTI', '/нефть': 'WTI',
    '/btc': 'BTCUSD', '/биткоин': 'BTCUSD'
  };

  if (quickQuoteCommands[command]) {
    const symbol = quickQuoteCommands[command];
    await bot.sendChatAction(chatId, 'typing', getActionOptions(threadId));
    
    const result = await ai.getPrice(symbol);
    
    if (result.error) {
      return bot.sendMessage(chatId, `⏳ ${result.message}. Попробуй через пару секунд.`, getBaseOptions(threadId));
    }
    
    return bot.sendMessage(chatId, result.text, { 
  ...
  if (command === '/restart' && userId === config.adminId) {
    ...
    exec('pm2 restart malysh-bot', (err) => {
  ...
  // === СТАТИСТИКА ===
  if (cleanText === 'малыш стата' || cleanText === 'малыш статистика') {
  ...
  if (hasTriggerWord) {
      // === 💰 КОТИРОВКИ (AllTick) ===
      const symbol = quotesService.detectSymbol(text);
      ...
      if (symbol) {
          ...
          if (result.error) {
              return bot.sendMessage(chatId, `⏳ ${result.message}. Попробуй через пару секунд.`, getReplyOptions(msg));
          }
      ...
      // Команда "Малыш, этот чат про..." — используем оригинальный текст (не lowercase)
      ...
      const isWhoGame = cleanText.match(/(?:кто|кого)\s+(?:из нас|тут|здесь|в чате|сегодня)/) || cleanText.match(/малыш\W+кто\??$/) || cleanText.trim() === "малыш кто";
      if (isWhoGame) {
          try { await bot.sendChatAction(chatId, 'typing', getActionOptions(threadId)); } catch(e){}
  ...
  // === РЕШЕНИЕ ОБ ОТВЕТЕ ===
  // Бот отвечает ТОЛЬКО когда его явно вызвали (тег "малыш/malysh") или ответили на его сообщение
  ...
  if (shouldAnswer) {
    ...
    if (!aiResponse) {
        ...
        aiResponse = getMalyshErrorReply("503 overloaded");
        ...
        aiResponse = getMalyshErrorReply(err.message);
    }
    ...
    if (aiResponse && typeof aiResponse === 'object' && aiResponse.type === 'image') {
        ...
        try {
            ...
            aiResponse = aiResponse.caption || "Не удалось отправить изображение";
        }
    }

    // Создаем копию текста для обработки
    let formattedResponse = typeof aiResponse === 'string' ? aiResponse : '';
    ...
    try {
        ...
        addToHistory(chatId, "Малыш", aiResponse);
      ...
      // ПРЕДОТВРАЩАЕМ АНАЛИЗ СОБСТВЕННОЙ РЕПУТАЦИИ
