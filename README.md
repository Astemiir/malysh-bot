...
### RAG и Умный Поиск
Малыш не галлюцинирует курсами валют за 2021 год. Если вы спросите про новости или цены, он полезет в интернет.
...
### Для всех
*   **Текст:** Просто общайтесь. Бот ответит, если его триггернуть (имя "Малыш") или если он решит, что это уместно.
*   `Малыш напомни [текст]` — поставит напоминание.
*   `Малыш кто я?` — расскажет ваше досье.
*   `Малыш кинь монетку` — орел/решка.
*   `/start` — справка.
...
*   `/unban ID` — Разбанить.
*   `Малыш стата` — Показать подробную статистику расхода токенов по провайдерам (API / Search / Native).
